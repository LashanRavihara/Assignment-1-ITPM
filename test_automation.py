...
def run_test():
    ...
    print(f"[DEBUG] Header values: {[str(v) if v is not None else '' for v in header_values]}")
    print(f"[DEBUG] Input col index: {input_col_idx}  ('{header_values[input_col_idx-1] if input_col_idx else 'N/A'}')")
    print(f"[DEBUG] Expected col index: {expected_col_idx}  ('{header_values[expected_col_idx-1] if expected_col_idx else 'N/A'}')")
    # Show first 3 data rows to verify
    for _dbg_r in range(header_row + 1, min(header_row + 4, int(ws.max_row or 0) + 1)):
        _cell_val = _merged_top_left_cell(ws, _dbg_r, input_col_idx).value if input_col_idx else None
        print(f"[DEBUG] Row {_dbg_r} input cell value: {repr(_cell_val)}")
    # ───────────────────────────────────────────────────────────────────────────
